...
### スプリント計画を策定したら必ず Notion に登録する

新しいスプリント計画を作成・確定したら、必ず Coarc の スプリント管理DB に登録すること。
RunWith Platform の開発管理には登録しない（プロジェクトが異なる）。

```
登録先DB: 🗂️ スプリント管理 | Coarc Platform
DB ID:    f44343cd-1421-4709-b6f1-7bd910681f99
DataSource: collection://75b8ab69-9520-45cb-b110-8793141eb807
```

### スプリント名の命名規則（ソート順に影響するため厳守）

DBはスプリント名の降順でソートされる。名前の形式が揃っていないとソートが崩れるため、
以下の形式を**必ず守ること**。

**✅ 正しい形式**

```
Sprint N｜テーマ（全角パイプ区切り・番号は0埋めなし）

例:
  Sprint 18｜AIフリーチャット + AIログ分析
  Sprint 19｜KPI目標管理 修正・実装
  Sprint 20｜顧客離反リスクAI
```
...
```
Sprint #18 — テーマ    ← #と半角ダッシュは使わない
Sprint18｜テーマ       ← 数字の前にスペースを入れる
Sprint 018｜テーマ     ← 0埋め不要
```

### 登録タイミング

- スプリント計画を確定した**直後**に登録する
- ステータスは計画時点で「🔜 計画中」、着手時に「🔵 進行中」、完了後に「✅ 完了」に変更する
